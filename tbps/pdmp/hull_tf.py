...
def sample_poisson_thinning(hull_samples, time):
  with tf.name_scope('sample_poisson_thinning'):
    ...
    print('hull samples = {}'.format(hull_samples))
    print('time samples = {}'.format(time))
    #print('S = {}'.format(S))
    #print('fS = {}'.format(fS))
    m_array, b_array, left_array, right_array = compute_hulls(
      S, fS, domain=[0.0, 10.0])
    # print('m arary shape = {}'.format(m_array))
    print('left array shape = {}'.format(left_array))
    print('right array shape = {}'.format(right_array))
    print('m_array = {}'.format(right_array))
    ...
    sample_time, hull_eval = sample_poisson(integrated_constant_array,
                                            integrated_range_lower,
                                            integrated_range_upper,
                                            inverse_integrated_domain_lower,
                                            inverse_integrated_domain_upper,
    ...
    # print(sample_time)
    return tf.cast(tf.reshape(sample_time, ()), tf.float32), tf.cast(tf.reshape(hull_eval, ()), tf.float32)
...
def compute_domain_range(m_array, b_array, left_array, right_array,
  ...
  with tf.name_scope('compute_domain_range'):
    ...
    m_array = m_array.write(index, tf.abs(m_array.read(index - 1) + 1e-6))
    ...
    inverse_integrated_domain_upper =  inverse_integrated_domain_upper.write(
      index, tf.reshape(tf.convert_to_tensor(100.0, dtype=tf.float64), ()))
...
def sample_poisson(integrated_constant_tensor,
  ...
  # print('shape of extr = {}'.format(tf.size(inverse_integrated_domain_upper)))
  ...
  poisson_sample, hull_eval = evaluate_hull_segment_inverse(
    E_sample, hull_segment_idx, m_tensor, b_tensor,  left_tensor,
    integrated_constant_tensor)
  #time.sleep(5)
  return poisson_sample, hull_eval
...
def evaluate_hull_segment_inverse(sample, hull_segment_idx, m_tensor,
                                  b_tensor, left_tensor, constant_tensor,
                                  epsilon=1e-6):
  """ will evaluate the inverse rate of this segment at `sample`"""
  # print('hull segment ind = {}, size = {}'.format(hull_segment_idx,
  #                                                 tf.shape(hull_segment_idx)))
  ...
  # ie. m = 0
  cond = tf.math.greater(tf.math.abs(m), -100)
  # cond = tf.math.greater(tf.math.abs(m), -100)
  ...
  print('cond small m = {}'.format(cond))
  eval_inv_segment = tf.cond(cond, m_valid, m_small)
  ...
  # print('m = {}, b = {}, integrated_constant = {}, integrated_lower = {}, constant = {}'.format(m, b, integrated_constant, integrated_lower, integrated_constant))
  # now evaluate the hull segment
  eval_hull_segment = m * eval_inv_segment + b
  print('m = {}, b = {}, c = {}, hull_segment = {}'.format(m, b, integrated_constant, eval_hull_segment))
  return eval_inv_segment, eval_hull_segment
