...
from tbnn.pdmp.resnet18 import ResNet18, ResNet20, ResnetBlock
from tbnn.pdmp.resnet import make_resnet, FLR_TLU
...
#       #p_scale = 1.0
#       kernel_dim = (2.0 * config['conv_param']['kernel_size'] *
#                     config['conv_param']['num_output'])
#       p_scale = tf.cast(tf.sqrt(2.0 / kernel_dim), dtype=tf.float32)
#     elif (config['type'] == 'dense'):
#       p_scale = tf.cast(tf.sqrt(2.0 / config['dense_param']['dim']),
#                                 dtype=tf.float32)
#     else:
...
def build_prior(model):
  weights_prior = []
  bias_prior = []
  for layer in model.layers:
    if isinstance(layer, tf.keras.layers.Conv2D):
      receptive_field_size = np.prod(layer.kernel_size)
      fan_in = layer.kernel.shape[-2] * receptive_field_size
      ...
      #p_scale = tf.cast(1.0, dtype=tf.float32)
      ...
      block_weight_priors, block_bias_priors = layer.resnet_block_prior()
      weights_prior.extend(block_weight_priors)
      bias_prior.extend(block_bias_priors)
    else:
      continue
  return weights_prior, bias_prior
...
#       # an incorrect layer type was supplied, so raise an error
#       err_str = 'Incorrect layer supplied here. expected layer of type'\
#                 'of either `dense` or `conv`, but got {}'.format(type(layer))
#       raise ValueError(err_str)
#   return weights_prior, bias_prior
...
def test_bnn_joint_log_prob_fn(model, weight_prior_fns, bias_prior_fns, X, y):
  def _fn(*param_list):
    with tf.name_scope('bnn_joint_log_prob_fn'):
      ...
      # now get the losses from the prior
...
#     print('-------------------------------------------')
#     print('layer = {}, type = {}'.format(i, model.layers[i]))
#     if isinstance(layer, tf.keras.layers.Conv2D):
#     if(isinstance(model.layers[i], tf.keras.layers.Conv2D) or
#        isinstance(model.layers[i], tf.keras.layers.Dense)):
#       print('kernel_shape = {}, bias_shape = {}'.format(
#         model.layers[i].kernel.shape, model.layers[i].bias.shape))
...
def main(args):
  ...
  #model = ResNet20(10)
  #_ = model(X_build)
  model = make_resnet(10, # num_classes
                      20, # depth
                      input_shape=(32, 32, 3))
  ...
  weight_prior_fns, bias_prior_fns = build_prior(model)
  ...
  if args.map_path == None:
    ...
    map_initial_state = get_map_iter(test_fn, initial_state, model,
                                     num_iters=args.map_iters, save_every=10000,
                                     dataset=training_iter,
                                     X_test=X_test,
                                     y_test=y_test,
                                     X_train=X_train,
                                     y_train=y_train)
