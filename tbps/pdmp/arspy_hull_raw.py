...
def compute_hulls(S, fS, domain):
  ...
  for li in range(1, len(S) - 2):
    ...
    if isinf(m1) or abs(m1 - m2) < 10.0 ** 4 * eps(m1):
      print('m1 is inf or m1 and m2 are too similar')
      print('m1 = {}'.format(m1))
      print('m2 = {}'.format(m2))
      print('abs(m1 - m2) = {}'.format(abs(m1 - m2)))
      print('10.0 ** 8 * eps(m1) = {}'.format(10.0 ** 8 * eps(m1)))
      ix = S[li]
      pr1 = float("-inf")
      ...
      if abs(ix - S[li]) < 10.0 ** 6 * eps(S[li]):
        ix = S[li]
      elif abs(ix - S[li + 1]) < 10.0** 6 * eps(S[li + 1]):
        ix = S[li + 1]

      print('S_i = {}, S_p1 = {}, ix = {}'.format(S[li], S[li + 1], ix))
