...
from tbnn.pdmp.model import (plot_density, get_model_state, pred_forward_pass,
                             ...
                             build_network,
                             bps_iter_main, hmc_main, nuts_main, boomerang_iter_main,
...
def boomerang_bnn_neg_joint_log_prob_fn(model, likelihood_fn, X, y, preconditioner, mean):
  def _fn(param_list, velocity_list):
    with tf.name_scope('bnn_joint_log_prob_fn'):
      # set the model params
      m = set_model_params(model, param_list)
      # print('current  model params ttttt= {}'.format(model.layers[1].kernel))
...
def main(args):
  ...
  if args.map_path == None:
    map_start = time.time()
    map_initial_state = get_map_iter(bnn_neg_joint_log_prob,
                                     initial_state, model,
                                     likelihood_fn,
  ...
  if args.boomerang:
    boomerang_iter_main(model, args.ipp_sampler, args.ref, args.num_results,
                        args.num_burnin, args.out_dir,
  ...
  if args.pbps:
    pbps_iter_main(model, args.ipp_sampler, args.ref, args.num_results,
                   ...
                   plot_results=~args.no_plot)
  if args.sgld:
    sgld_iter_main(model, args.ipp_sampler, args.ref, args.num_results,
                  args.num_burnin, args.out_dir,
                  bnn_neg_joint_log_prob, map_initial_state,
...
if __name__ == '__main__':
  ...
  parser.add_argument('--boomerang', type=bool, default=False, nargs='?',
                      const=True, help='whether to run Boomerang Sampler')
